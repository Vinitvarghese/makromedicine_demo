...
class Webcoder_Model extends CI_Model
{
    ...
    */
    ...
    public function getApliedUsers($company_id, $user_id, $approved){
        ...
        $this->db->join('wc_person_type_translation AS tr', 'tr.person_type_id = rel.role_id AND tr.language_id=1', 'inner');
    ...
    public function checkCompanyLogo($data){
        ...
        return (!empty($data->company_logo) && file_exists($img)) ? site_url().$img : base_url('templates/default/assets/images/bloomberg.png');
    }


    public function getPageDataByLink($link){
        $this->db->select('*');
        $this->db->from('wc_companies');
    ...
    public function getUserPermissionByPage($user_id, $company_id){
        ...
        $this->db->join('wc_person_type_translation AS tr', 'tr.person_type_id = rel.role_id AND tr.language_id=1 AND rel.delete_at IS NULL', 'inner');
        $this->db->join('wc_companies AS c', "c.id=rel.company_id",  'inner');
        $this->db->group_by("rel.id");
        $query = $this->db->get();
    ...
    public function getUserCompanyAndRoles($user_id){
        $this->db->select('
            ...
            (SELECT COUNT(id) FROM wc_user_notify WHERE company_id=c.id AND send_id!="'.$user_id.'" AND `status`=1) AS `page_notif_count`,
