...
class Product extends Admin_Controller
{
  	...
  	public function index()
    {
          ...
          // Table Column
          $this->data['fields'] = ['id','title', 'company_name','email','checked',  'pr_type', 'atc_code', 'packing_type', 'country', 'medical_cl'];
          ...
          if ($this->input->get('checked') != NULL) {
              $filter['wc_product.checked'] = $this->input->get('checked');
          }
          $sort = [
              'column' => ($this->input->get('column')) ? $this->input->get('column') : 'created_at',
              'order' => ($this->input->get('order')) ? $this->input->get('order') : 'DESC'
          ];
          $this->data['total_rows'] = $this->{$this->model}->fields('wc_product.*,wc_companies.company_name')
              ->join('wc_users', 'wc_product.user_id = wc_users.id','left')
          ...
          $filter['wc_product.user_id in (select id from wc_users where deleted_at is null)']=NULL;
          $filter['wc_product.deleted_at is null']=NULL;

          $this->data['message'] = ($this->session->flashdata('message')) ? $this->session->flashdata('message') : '';
          $total_rows = $this->{$this->model}->fields('wc_product.*,wc_companies.company_name')
              ->join('wc_users', 'wc_product.user_id = wc_users.id','left')
              ->join('wc_company_user_rel', 'wc_company_user_rel.user_id = wc_users.id AND delete_at IS NULL','left')
              ->join('wc_companies', 'wc_companies.id = wc_company_user_rel.company_id','left')
              ->where($filter)->with_translation()->get_count_rows(1);

          $rows = $this->{$this->model}->fields('wc_product.*,wc_product_translation.title,wc_product_translation.alias, wc_companies.company_name')
              ->join('wc_users', 'wc_product.user_id = wc_users.id','left')
              ->join('wc_company_user_rel', 'wc_company_user_rel.user_id = wc_users.id AND delete_at IS NULL','left')
              ->join('wc_companies', 'wc_companies.id = wc_company_user_rel.company_id','left')
          ...
          if($rows){
                    ...
                    foreach ($rows as $value) {
                       ...
                       if (count($atc_code) > 0){
                        ...
                        if (!empty($value->medical_cl) && $value->medical_cl!='') {
                          foreach (get_selected_medical($value->medical_cl) as $key=>$val){
                            $medical_cl.= '<b>'.$val->name.'</b>';
                          } 
                        }

                        $array_data[] = (object) [
                         'id'      => $value->id,
                          'title'   => '<a target="_blank" href="'.site_url_multi('product/view/').$value->id.'-'.$value->alias.'">'.$value->title.'</a>',
          ...
          $this->data['breadcrumb_links'][] = [
              ...
              'label_value' => $this->db->where('deleted_at IS NOT NULL',NULL)->from('wc_product')->count_all_results(),
  	...
  	{
      $user_id_query = $this->db->select('user_id')->where('id',$id)->get('wc_product');
    ...
    public function changeStatus($id=false)
    {
      $id = $this->input->post('id');
       if($id != false)
      {
      $pr = $this->db->where('id', $id)->get('wc_product')->result();
      if($pr[0]->checked == 1){
         $var = $this->db->set('checked', '0', FALSE)->where('id', $id)->update('wc_product');
          redirect(site_url_multi($this->directory . $this->controller.'?checked=0'), 'refresh');
      }
      else{

        $var = $this->db->set('checked', '1', FALSE)->where('id', $id)->update('wc_product');
            if($var){
              $userid = $this->{$this->model}->fields(['user_id','title'])->filter(['id'=>$id])->with_translation()->one();
              $this->data['notify_data']  = [
                'user_id'       => $userid->user_id,
                  'company_id'       => $pr[0]->company_id,
                'send_id'       => 1,
                'status'        => 1,
                'sender'        => 1,
                'type'          => 3,
