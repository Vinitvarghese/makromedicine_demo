...
class Companies extends Admin_Controller {
    ...
    {
        if (isset($_POST['role_id']) && isset($_POST['role_main_id'])){
            $this->changeUserRole();
            exit;
        }

        if (isset($_POST['removeUserFromPage']) && isset($_POST['role_main_id'])){
            $this->removeUserFromPage();
            exit;
        }

        if (isset($_POST['updateApproveStatus']) && isset($_POST['company_id'])) {
        ...
        $this->data['companies'] =$this->{$this->model}->getCompanies();
        $this->data['user_page_roles']=$this->{$this->model}->getPageRoles();
    ...
    public function changeUserRole(){
        ...
        $role_main_id=$_POST['role_main_id'];

        $this->db->set(['role_id' => $role_id, 'updated_at' => date('Y-m-d H:i:s')], FALSE);
        $this->db->where('id', $role_main_id);
        $this->db->update('wc_company_user_rel');

        echo json_encode(['success' => true]);
    }

    public function removeUserFromPage(){
        $role_main_id=$_POST['role_main_id'];
    ...
    public function updateApproveStatus()
    {
        ...
        $approved_st = $_POST['approved_st'];

        $this->db->set(['status' => $approved_st, 'checked' => $approved_st, 'updated_at' => date('Y-m-d H:i:s')], FALSE);
        $this->db->where('id', $company_id);
        $this->db->update('wc_companies');

        $this->db->set(['status' => $approved_st, 'updated_at' => date('Y-m-d H:i:s')], FALSE);
        $this->db->where('company_id', $company_id);
        $this->db->update('wc_confirm_account');
