...
class Module extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		if ($this->module_setting) {
			//if ($this->module_setting->status == 1) {
				if (in_array($this->method, json_decode($this->module_setting->methods, true)) || $this->method == 'changeStatus' ) {
					if (check_permission($this->module_name, false,true) || $this->method == 'changeStatus') {
						...
						if (in_array($this->method, ['index', 'trash', 'create', 'edit', 'show'])) {
							...
							$thismethod = ($this->method == 'show')? 'index' : $this->method;
							$this->data['index_title']    = valid_lang(json_decode($this->module_setting->name)->{$thismethod}->title);
							$this->data['title']          = valid_lang(json_decode($this->module_setting->name)->{$thismethod}->title);
							$this->data['subtitle']       = valid_lang(json_decode($this->module_setting->name)->{$thismethod}->subtitle);
	...
	public function index()
	{
		...
		if ($this->module_setting->search_field) {
			...
			if ($this->input->get($this->module_setting->search_field) != null) {
				$fl_arr = explode(',',$this->module_setting->search_field);
				if(count($fl_arr)>1){
					$like_str='';
					foreach ($fl_arr as $fl) {
						$like_str.=$fl.' like '. " '%" . $this->input->get($this->module_setting->search_field) . "%' or ";
					}
					$like_str = rtrim($like_str, 'or ');
					$like_str = '('.$like_str.')';
					$this->data['filter'][$like_str] = NULL;
				}
				else{
		...
		$sug_count_query = $this->Suggestion_model->filter(['type' => $this->module_setting->slug])->get_count_rows();
		if($sug_count_query) {

			$sug_count = $sug_count_query;
			if($sug_count>0){ 
				$this->data['sug_count']=$sug_count;
				$this->data['sug_url']=base_url('admin/suggestion?type='.$this->module_setting->slug);
			}
		}
		...
		if ($this->module_setting->multilingual == 1) {
			$rows = $this->{$this->model}->fields($this->data['fields'])->with_translation($language_id)->order_by($sort['column'], $sort['order'])->limit($per_page, ($page-1)*$per_page)->all($this->data['filter']);
		} else {
			$rows = $this->{$this->model}->fields($this->data['fields'])->order_by($sort['column'], $sort['order'])->limit($per_page, ($page-1)*$per_page)->all($this->data['filter']);
		}
		...
		$action_buttons = [];
		if (check_permission($this->module_setting->slug, 'show',true)) {
			$action_buttons['show'] = true;
		}


		if (check_permission($this->module_setting->slug, 'edit',true)) {
			$action_buttons['edit'] = true;
		}

		if (check_permission($this->module_setting->slug, 'delete',true)) {
		...
		if (in_array('create', json_decode($this->module_setting->methods, true))) {
			if (check_permission($this->module_name, 'create',true)) {
				$this->data['buttons'][] = [
					'type'  => 'a',
		...
		if (in_array('delete', json_decode($this->module_setting->methods, true))) {
			if (check_permission($this->module_name, 'delete',true)) {
				$this->data['buttons'][] = [
					'type' => 'button',
	...
	public function trash()
	{
		...
		if (check_permission($this->module_setting->slug, 'restore',true)) {
			$action_buttons['restore'] = true;
		}

		if (check_permission($this->module_setting->slug, 'remove',true)) {
		...
		if (in_array('remove', json_decode($this->module_setting->methods, true))) {
			if (check_permission($this->module_name, 'remove',true)) {
				$this->data['buttons'][] = [
					'type' => 'button',
		...
		if (in_array('restore', json_decode($this->module_setting->methods, true))) {
			if (check_permission($this->module_name, 'restore',true)) {
				$this->data['buttons'][] = [
					'type' => 'button',
		...
		if (in_array('clean', json_decode($this->module_setting->methods, true))) {
			if (check_permission($this->module_name, 'clean',true)) {
				$this->data['buttons'][] = [
					'type' => 'a',
			...
			if ($general_field->show_on_form == 1) {
				...
				if (in_array($general_field->element, ['text', 'slug', 'password', 'email', 'number', 'tel', 'url', 'range', 'search', 'date', 'datetime', 'time', 'color', 'month', 'week', 'submit', 'reset', 'button', 'radio', 'textarea'])) {
					...
					if(isset($general_field->module)) {
						$options = $this->generate_options($general_field->module);
					} elseif(isset($general_field->options)) {
						foreach ($general_field->options as $option) {
							$options[$option->key] = $option->value;
						}
					}
		...
		if ($this->input->method() == 'post') {
			if ($this->form_validation->run() == true) {
				...
				if (!empty($general)) {
					...
					if($this->model == 'cas_list_model'){
						$tr_array=$this->input->post('translation');
						$casno = $tr_array[1]['cas_no'];
						$get_cas = $this->db->where('cas_no',$casno)->from('wc_cas_list_translation')->count_all_results();
						if($get_cas>0) $this->data['message'] = 'Cas Number already exists';
						else{
					...
					$this->insert_log($id,$this->model,'insert');
				...
				if (isset($form_fields->general) && !empty($form_fields->general)) {
					foreach ($form_fields->general as $general_field) {                        
						if ($general_field->show_on_form == 1) {
							...
							if($this->module_setting->slug == 'suggestion'){
								$isCas = false;
								foreach ($this->data['form_field']['general'] as $key => $val) {
									if(isset($val['value']) && $val['value'] == 'cas_list'){
										$isCas= true;
									}
								}
								if($isCas){
									foreach ($this->data['form_field']['general'] as $key => $val) {
										if($val['label'] == 'Text'){
											$this->data['form_field']['general'][$key]['label']='Cas no';
										}
										else if($val['label'] == 'Text2'){
											$this->data['form_field']['general'][$key]['label']='Molecular formula';
										}
									}
								}
							}
				...
				if ($this->input->method() == 'post') {
					if ($this->form_validation->run() == true) {
						...
						if (!empty($general)) {
							...
							if($this->model == 'suggestion_model' && $general['status']==1){
									if($general['type'] == 'cas_list'){
										$name = $general['name']; 
										$text = $general['text']; 
										$text2 = $general['text2'];
								          ...
								          $insert_c = $this->db->insert('wc_cas_list', $data_cas);

								          $insert_id = $this->db->insert_id();

								          $data_cas = [
								            'cas_list_id' => $insert_id,
								            'language_id' => 1,
								            'chemical_name' => $name,
								            'cas_no' => $text,
								            'molecular_formula' => $text2
								          ];

								          $insert_c = $this->db->insert('wc_cas_list_translation', $data_cas);


									}
									else if($general['type'] == 'herb_part'){
										$name = $general['name']; 
										$text = $general['text']; 
								          ...
								          $insert_c = $this->db->insert('wc_herb_part', $data_cas);

								          $insert_id = $this->db->insert_id();

								          $data_cas = [
								            'herb_part_id' => $insert_id,
								            'language_id' => 1,
								            'name' => $name,
								            'description' => $text
								          ];

								          $insert_c = $this->db->insert('wc_herb_part_translation', $data_cas);
							...
							$this->insert_log($id,$this->model,'update');
						...
						if ($general_field->show_on_form == 1) {
							...
							if (in_array($general_field->element, ['text', 'password', 'email', 'number', 'tel', 'url', 'range', 'search', 'date', 'datetime', 'time', 'color', 'month', 'week', 'submit', 'reset', 'button', 'radio', 'checkbox', 'textarea'])) {
								...
								if (isset($general_field->module) && !empty($general_field->module)) {
									...
									$options  =$row->{$general_field->column};
								}
	...
	public function delete($id = false)
	{
		if ($id) {
			...
			$this->insert_log($id,$this->model,'delete1');
		} else {
			if ($this->input->method() == 'post') {
	...
	public function clean()
	{
		...
		$this->insert_log($id,$this->model,'delete_all');
	...
	public function changeStatus()
	{
		if ($this->input->method() == 'post') {
			...
			if ($id){
				...
				if($this->model == 'suggestion_model' && $status==1){
					if($row->type == 'cas_list'){
						$name = $row->name; 
						$text = $row->text; 
						$text2 = $row->text2;
	...
	public function generate_options($module){
		...
		//if(isset($_GET['test1'])){
			//var_dump($this->{$model_name});echo '--'; var_dump($order);
		//}
		if($model_name == 'Country_model' || $model_name == 'Continent_model') {$order=array();$order['name']='ASC';}
	...
	public function insert_log($id,$thismodel,$type){
		/*$ip_address = $this->input->ip_address();

		$data22 = [];
		$data22['updated_table']=str_replace('_model', '', $thismodel);
		$data22['updated_id']=$id;
		$data22['operation']=$type;
		$data22['ip_address'] = $ip_address;
		$data22['username'] = $_SESSION['username'];
		$data22['timestamp'] = date("Y-m-d H:i:s");
		$insert = $this->db->insert('hst_admin_logs', $data22);
		return $insert;*/
	}
