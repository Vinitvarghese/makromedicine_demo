...
class Site_Controller extends Webcoder_Controller
{
    ...
    public function __construct() {
        ...
        $check_group_type=($this->uri->segment('4') == 'interests' || 
        $this->uri->segment('3') == 'create-page' || $this->uri->segment('5') == 'edit-page' ||
        $this->uri->segment('2')==''
        ) ? 2 : 1;
        ...
        $all_groups=$this->Group_model->fields(['id', 'name'])->filter(['type' => $check_group_type])->all();

        $this->data['groups']= ($check_group_type==2 && $this->uri->segment('2') != '') ? (object) array_merge((array) $def_data, (array) $all_groups) : $all_groups;
       ...
       $this->data['parent_atc']       = $this->Atc_code_model->fields(['id', 'atc_code', 'id_parent', 'meaning'])->order_by('sort', 'ASC')->filter(['CHAR_LENGTH(atc_code)'=> 1])->with_translation()->all();
        ...
        $this->data['industries']=$this->User_model->getIndustries();
        ...
        if ($this->auth->is_loggedin()) {
            ...
            if(isset($this->auth->get_user()->id))
            {
              ...
              if(!empty($page_data)){
                    ...
                    if ($page_data->create_company==0) {
                        echo "<script>
                            alert(\"Your company doesn't have permission for this page\");
                            window.location.href='". site_url_multi('/')."';
                        </script>";
    ...
    public function sendMail($to_emails=[], $subject, $message)
    {
