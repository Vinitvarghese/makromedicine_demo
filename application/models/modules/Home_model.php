...
class Home_model extends Webcoder_Model {
	...
	public function getProductCount() {
		$query = $this->db->query( "SELECT country, COUNT(id) FROM wc_product left join wc_product_translation on wc_product_translation.product_id=wc_product.id where title is not null and checked=1 and deleted_at is null and user_id in(select id from wc_users where checked=1 and status=1 and deleted_at is null) GROUP BY country" );
