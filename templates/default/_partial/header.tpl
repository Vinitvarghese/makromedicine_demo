...
<head>
    ...
    <script type="text/javascript"> var site_url = "{base_url()}", redirect_url="{site_url_multi('/')}"; </script>
    {$js}
    <script>
                ...
                success: function (data) {
                    ...
                    if (obj[0].login == 'success') {
                        if (obj[0].type == 'success') {
                            ...
                            window.location = redirect_url + 'profile/settings/';
                        } else {
                            $.each(obj, function (key, value) {
    ...
    <script>
        ...
        function phoneInput() {
            $('.phone').each(function () {
                var bu=$(this),
                    country_code=$.trim(bu.data('co')),
                    initialCountry=(country_code.length > 0) ? country_code  : "auto";

                if(initialCountry=='+994'){
                    initialCountry='az';
                }
                                ...
                                <ul class="dropdown not_and_msg_list">
                                    ...
                                    <div class="profile-thump">
                                        ...
                                        <div class="right-profile-thump">
                                            <a href="#" data-toggle="dropdown" title="{$UserData->fullname}">
                                                <img src="{$user_images}" alt="{$UserData->fullname}">
                                ...
                                <div class="col-md-9 xs-no-padding advanced-pag">
                                    <div class="search-menu">
                                        ...
                                        <select name="search_type" class="selectpicker show-menu-arrow main_search_type"
                                            ...
                                            <option value="3" {if $search_type == 3 || $search_type == 5} selected="selected" {/if}>Product
                                            </option>
                                            ...
                                            {if $continents}
                                                {foreach from=$continents item=continent}
                                                    <option value="{$continent->code}"
                                                        {if in_array($continent->code, $search_continent)} selected="selected"
                                                        {/if}>{$continent->name}</option>
                                                {/foreach}
                                            {/if}
                                        </select>
                                        <div class="country-inner btn-group bootstrap-select show-tick show-menu-arrow">
                                        </div>
                                        <select name="content_type[]"
                                            class="selectpicker show-menu-arrow main_content_type" multiple
                                            data-live-search="true" data-actions-box="true"
                                            data-selected-text-format="count > 0" title="Content Type">
                                            <option value="0" {if in_array(0, $content_types)} selected="selected"
                                                {/if}>Monocomponent</option>
                                            <option value="1" {if in_array(1, $content_types)} selected="selected"
                                                {/if}>Policomponent</option>
                                        </select>
                                        ...
                                        <select name="standart[]"
                                            ...
                                            {if $standarts}
                                                {foreach $standarts as $key => $value}
                                                    <option value="{$value->id}" {if in_array($value->id, $search_standart)}
                                                        selected="selected" {/if}>{$value->name}</option>
                                                {/foreach}
                                            {/if}
                                        </select>
                                        <select name="atc_classifiction[]"
                                                    ...
                                                    <optgroup label="{$parent->atc_code} - {$parent->meaning}"
                                                        ...
                                                        {if $list_atc[$parent->id]}
                                                            {foreach $list_atc[$parent->id] as $child}
                                                                <option value="{$child->atc_code}"
                                                                    {if in_array($child->atc_code, $atc_classifiction)}
                                                                    selected="selected" {/if}>{$child->atc_code} - {$child->meaning}
                                                                </option>
                                                            {/foreach}
                                                        {/if}
                                                    </optgroup>
