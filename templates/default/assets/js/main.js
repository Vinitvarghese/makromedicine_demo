...
$(document).ready(function() {
        ...
        if (data != "danger") {
          ...
          country_sel.selectpicker({
            countSelectedText: function(num) {
              ...
              if (num > 0) {
                return num+ " items selected"; //title + "(" + num + ")";
              }
            }
          });
        } else {
  ...
  $(document).ready(function(){
    $(".advanced-pag .show-menu-arrow.main_search_type").trigger('change');
  })
        ...
        $.each(data, function(key, value) {
          if (types == "chemical") {
            ...
            clone +=
                '<div class="lib-span4">' +
                value.chemical_name +
                '</div><div class="lib-span5" >'+value.molecular_formula+'</div>';
            clone += "</a>";
            clone += '<div class="clearfix"></div>';
            clone += "</li>";
      ...
      success: function(data) {
        ...
        if (obj[0].login == "success") {
          if (obj[0].type == "success") {
            ...
            window.location = redirect_url + "profile/";
          } else {
            $.each(obj, function(key, value) {
  ...
  $(document).on("click", ".remove-item:not(.remove-c-row)", function() {
    ...
    if (confirm("Are you sure?")) {
      $.ajax({
        url: site_url+"profile/delete_interest/",
        type: "POST",
        data: {
          id: id
        }
  ...
  $("#validateRegister").validate({
    ...
    messages: {
      ...
      repassword: {
        ...
        minlength: "Repeat password must be at least 5 characters long"
      }
    }
  });
...
function phoneMask(){
  $(document).on('keypress', '.phone, .phones', function (e) {
...
function onlyAlphabet(){
  $(document).on('keypress', '.onlyalphabet', function (e) {
    var charCode=(window.event) ? window.event.keyCode : e.which;
  ...
  if (email.length > 0) {
    $.ajax({
      ...
      success: function(res) {
        ...
        if(res[0].type=="succsess"){
          toastr.success("The link has been sent to your mail address");
          $("#forgot_password_messge").text("The link has been sent to your mail address");
          $("#forgetPassword").modal("hide");
        }else{
          toastr.error("This mail address not exits");
        }
      }
    });
