                ...
                <div class="n_right_section start_with_text employee_l">
                    ...
                    <div class="full_width flex justify_between align_center all_news_list_top">
                        ...
                        <div class="flex align_center">
                            ...
                            <ul class="flex all_news_list_tabs">
                                <li><a href="{site_url_multi('/')}pages/{$UserData->slug_user}/following?type=2" class=" {if $check_type==2} active {/if} ">Companies</a> </li>
                                    <li><a href="{site_url_multi('/')}pages/{$UserData->slug_user}/following?type=1"
                                            class=" {if $check_type==1} active {/if} ">Users</a> </li>
                            </ul>
                        </div>

                        <span class="blocked_user_count">{count($following_users)} {if $check_type==1} Users {else} Companies {/if}</span>
                    ...
                    <ul class="full_width flex flex_wrap user_list blocked_user_list">
                        {if !empty($following_users)}
                            {foreach $following_users as $k => $v}
            ...
            $(document).on("click", ".un_block_user", function (e) {
                ...
                if (!e.handle){
                    ...
                    if (confirm("Are you sure?")){
                        ...
                        $.ajax({
                            url :  redirect_url+'pages/{$UserData->slug_user}/following',
                            type : 'POST',
