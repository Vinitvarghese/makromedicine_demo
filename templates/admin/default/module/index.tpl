...
<div class="panel panel-white">
	<div class="panel-heading">
		...
		{if isset($sug_count)}<a href="{$sug_url}">Suggestions ({$sug_count})</a>{/if}
		<div class="heading-elements">
			<div class="btn-group">
				{if isset($language_list_holder) && is_array($language_list_holder)}
                    {foreach $language_list_holder as $language}		
                        <a href="{site_url_multi($admin_url)}/{$module_name}?language_id={$language.id}" class="{$language.class}"><img src="{$admin_theme}/global_assets/images/flags/{$language.code}.png" alt="{$language.name}"> {$language.name|upper} <span class="label bg-slate-700">{$language.count}</span></a>
                    {/foreach}
				{/if}
			</div>
